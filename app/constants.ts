...
export const calculateTotalPrice = (priceBeforeVat: number, vat: number): number => {
    return Math.round(priceBeforeVat * (1 + vat / 100));
};

export const getPopularityBadge = (size: number): string | null => {
    if (size === 8 || size === 12) return "Most Popular";
    if (size === 6 || size === 10) return "Great Value";
    if (size === 20 || size === 40) return "Commercial";
    return null;
};

export const getSkipImage = (size: number): string => {
    if (size <= 8) return "https://yozbrydxdlcxghkphhtq.supabase.co/storage/v1/object/public/skips/skip-sizes/4-yarder-skip.jpg";
    if (size <= 16) return "https://yozbrydxdlcxghkphhtq.supabase.co/storage/v1/object/public/skips/skip-sizes/8-yarder-skip.jpg";
